...
fn main() {
    ...
    // Clone the receiver so each child thread gets one end.
    let rx_child1 = rx_challenge.clone();
    let rx_child2 = rx_challenge;

    let child_thread1 = thread::spawn(move || {
        // Loop over messages received and print them.
        for msg in rx_child1 {
            println!("Child thread 1 received: {}", msg);
        }
    });

    let child_thread2 = thread::spawn(move || {
        // Loop over messages received and print them.
        for msg in rx_child2 {
            println!("Child thread 2 received: {}", msg);
        }
    });

    // In the main thread, send several values (printing each before sending).
    for i in 1..=5 {
        let msg = format!("Message {}", i);
        println!("Main thread sending: {}", msg);
        tx_challenge.send(msg).unwrap();
        pause_ms(100);
    }

    // Close the sending side to signal no more messages.
    drop(tx_challenge);

    // Wait for both child threads to finish processing.
